...
describe("transparent desktop experiment", () => {
  ...
  it("lets one driver group attempt and reject a rescue", async () => {
    ...
    expect(await page.locator("[data-rescue-result]").getAttribute("data-result-highlight")).toBe("true");
    expect(Number.parseFloat(await page.locator("[data-rescue-result]").evaluate((element) => getComputedStyle(element).outlineWidth))).toBe(3);
  ...
  it("offers a deliberate comparison before revealing the paradox", async () => {
    ...
    expect(await page.locator("[data-start-rescue]").evaluate((element) => getComputedStyle(element).outlineStyle)).toBe("none");
  ...
  it("closes and reopens the shortcut as a visible reversal", async () => {
    // Reduced motion so the toggle's scrollIntoView({behavior: "smooth"}) resolves
    ...
    // block: "start" can't always flush an element's top to 0 (e.g. near the
    // bottom of the page, where the scroll container runs out of room), so
    // check it's within the viewport rather than exactly at its top edge.
    await page.waitForFunction(() => {
      const rect = document.querySelector("[data-map-proof]")!.getBoundingClientRect();
